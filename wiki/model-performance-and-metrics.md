...
## Four-Table Performance Reporting & Ground Truth Mining (August 10)

Key performance analysis protocols established during the August 10 sync (source: Iris Sync - 2026_08_10):
- **Four-Table Verification Mandate**: To test whether post-processing filters (confidence thresholds, minimum bounding sizes) can elevate precision without degrading recall, Sachin was tasked with sharing four performance tables on Slack:
  1. *Unprocessed Results Tables* (raw model predictions across folds/models).
  2. *Filtered/Post-Processed Results Tables* (showing precision/recall shifts after rule-based filtering).
- **False Positive Ground Truth Mining**: Visual inspection of false positives generated by the high-recall V4 model revealed that many flagged objects resemble valid UXO contacts that were omitted from ground truth annotations or dropped from `AOI small black`.
- **Labeling Error Estimation**: Ground truth annotation omission rate was estimated at ~1–5%. The team confirmed that high-recall models serve as an effective mining mechanism to identify un-annotated targets, and approved a final cleaning pass so that precision gains reflect accurate ground truth rather than labeling omissions.

## V4 AUG + Copy-and-Paste Benchmark Metrics (August 12)

Sachin evaluated the V4 model combining image augmentations and copy-and-paste on the clean, verified single 80/20 train/validation split with remediated ground truth (~80 mined UX annotations added to `AOI small black`) (source: Iris Sync - 2026_08_12):
- **Merged Metaclass (`UXO` + `AOI small black`)**: Achieved an **F1 score of 92% (Precision ~94%, high recall)**.
- **Isolated `UXO` Class**: Achieved an **F1 score of 74%**.
- **Reporting Requirement**: Sachin was tasked with visually verifying detections and documenting the confusion matrix and prediction images in an HTML summary report within 24 hours.

## Procedural Synthetic-Only Model Recall (August 12)

Pratyaksh trained a model exclusively on procedurally generated synthetic labels with no real UXO samples (source: Iris Sync - 2026_08_12):
- **UXO Target Classes**: Achieved **50%–60% recall on UXO classes** (Class 6: actual UXO, Class 3: UXO-like), with precision ~46–47%.
- **Support Classes**: Precision and recall remained low on larger support structures (`AOI support`, `AOI big`) due to the absence of synthetic support training data.

## Metrics Clarifications & 4-Bucket Deliverables (August 14)

Performance analysis and customer reporting strategy from the August 14 sync (source: Iris Sync - 2026_08_14):
- **Correction on Previously Reported 90%+ F1 Score**: Sachin clarified that the 90%+ F1 score reported on the 80/20 split was artificially inflated because the evaluation script inadvertently merged `AOI big` and `black patch` into a single class alongside `UXO` + `AOI small black`.
- **Poisson Copy-Paste + Procedural Mine Synergy**:
  - Training with Poisson copy-paste combined with procedural mine augmentation achieved the highest recall on UXO classes, missing only 1 to 2 objects across validation sets.
  - Object-level F1 score at 0.1 IoU was ~67% due to aggressive high-recall detections (generating small false-positive contact detections that can be filtered via post-processing size and confidence thresholds).
  - BCE + Dice loss outperformed Tversky loss in this pipeline configuration.
- **Customer Deliverable Standard (4-Bucket Confusion Matrices)**: The team reaffirmed that single aggregate F1 numbers can mislead client expectations. Deliverables for Bedrock will center on **4-bucket confusion matrices** (presenting both raw object counts and percentages: UXO-like [UXO + AOI small black], Non-UXO-like [AOI Big, etc.], and Background) to demonstrate high recall on true targets while framing false positives clearly.
- **Test-Time Scaling / Multi-Model Inference Option**: Evaluated running multiple model variations or applying destructive augmentations (e.g. image inversion) in series at test time to boost precision without requiring parallel GPU compute. Agreed to explore this as an optional "test-time scaling" feature for Bedrock if experimental signals remain positive.
...
**Sources**: raw/Internal Bedrock x CrescerAI Initial SOW.md; raw/SOW 1 Milestone 2 Presentation.pptx; raw/Bedrock SOW 2.md; raw/meeting_transcripts/Iris Sync - 2026_07_10 through 2026_08_14; raw/meeting_transcripts/Iris Sync - 2026_08_07 12_16 EDT - Notes by Gemini.md; raw/meeting_transcripts/Iris Sync - 2026_08_10 12_27 EDT - Notes by Gemini.md; raw/meeting_transcripts/Iris Sync - 2026_08_12 12_27 EDT - Notes by Gemini.md; raw/meeting_transcripts/Iris Sync - 2026_08_14 12_30 EDT - Notes by Gemini.md
